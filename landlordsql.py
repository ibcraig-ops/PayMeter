...
def gen_executive_sales_report_pdf(summary_df, total_metrics, period_label, portfolio_label, logo_path="logo.png"):
    ...
    pdf.set_fill_color(30, 58, 138) # Corporate Deep Navy
    pdf.rect(0, 0, 297, 32, 'F')
    
    # Embed Logo inside Header if it exists
    if os.path.exists(logo_path):
        try:
            pdf.image(logo_path, x=12, y=5, h=22)
        except:
            pass
    ...
    # --- METADATA PANEL ---
    pdf.set_text_color(51, 65, 85) # Slate Charcoal
    ...
    pdf.set_fill_color(241, 245, 249) # Light Tint Background
    pdf.set_draw_color(226, 232, 240)
    
    # Draw KPI 1 (Gross Sales)
    ...
    # Columns Width Allocation (Sum = 273mm)
    ...
    # Render Table Rows
    pdf.set_text_color(51, 65, 85)
    pdf.set_draw_color(226, 232, 240) # Subtle border color
    ...
    for _, r in summary_df.iterrows():
        ...
        # Zebra striping background check
        if toggle_fill:
...
# --- 6. DATABASE CORE RETRIEVAL LOGIC ---
def load_users():
    try:
...
# --- 8. GLOBAL NAVIGATION SIDEBAR MENU ---
...
# --- 10. SYSTEM APPLICATION PAGES CONTROLLER ---
...
if st.session_state['current_page'] == "Dashboard":
    if fdf.empty: st.warning("No data matches selected timeline or building parameters.")
        ...
        st.dataframe(summary.style.format({'Sales': 'R {:,.2f}', 'Consumption': '{:,.2f}'}), use_container_width=True)
...
# --- NEW FULL FEATURE: EXECUTIVE REPORTING SUITE ---
elif st.session_state['current_page'] == "Reporting":
    st.title("🗂️ Executive Reporting Suite")
    if fdf.empty:
        st.info("Please adjust filters in the left sidebar configuration panel to include transaction variables.")
    else:
        st.markdown("### 📊 Financial Revenue & Sales Report Factory")
        st.write("Generate audits detailing gross financial allocations, principle share payouts, service fee structures, and metric calculations across the specified parameters.")
        
        # Build Analytical Consolidated Metrics Dataframe
        rpt_grouped = fdf.groupby(['Display_Month', 'Year_Month_Key', 'Building Detail', 'Service Resource']).agg({
        ...
        # Format columns cleanly for table display
        rpt_display = rpt_grouped.rename(columns={
        ...
        # Global Aggregates Dictionary for UI cards & PDF Generator
        totals = {
        ...
        # High-level Metrics Panel Layout
        ...
        st.dataframe(rpt_display.drop(columns=['Year_Month_Key']).style.format({
            'Gross Sales': 'R {:,.2f}',
        ...
        # Document Export Control Center
        ...
        portfolio_label = str(st.session_state['sel_owner'])
        
        with exp_col1:
            # A: Professional Excel compilation via pandas
            xl_buffer = io.BytesIO()
            with pd.ExcelWriter(xl_buffer, engine='openpyxl') as xl_writer:
                rpt_display.drop(columns=['Year_Month_Key']).to_excel(xl_writer, index=False, sheet_name="Sales Summary Report")
            st.download_button(
                label="📥 Export Report as Excel Ledger",
                data=xl_buffer.getvalue(),
                file_name=f"Sales_Summary_Report_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
            
        with exp_col2:
            # B: Clean Landscape FPDF layout design with embedded branding
            if FPDF:
                pdf_bytes = gen_executive_sales_report_pdf(
                    summary_df=rpt_display,
                ...
                if pdf_bytes:
                    st.download_button(
                        label="📥 Export Executive PDF Statement",
                        data=pdf_bytes,
                        file_name=f"Executive_Sales_Report_{datetime.now().strftime('%Y%m%d')}.pdf",
                        mime="application/pdf",
                        use_container_width=True
                    )
                else:
                    st.error("Error generating PDF statement payload.")
            else:
                st.warning("FPDF dependency missing. Please execute 'pip install fpdf2' to generate PDF statements.")
    ...
    if working_df.empty: st.warning("No tracking data available.")
        ...
        with col1: query_meter = st.text_input("Search Meter", placeholder="Filter digits...").strip()
